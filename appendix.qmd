...
BCI %>% 
 ...
 BCI_50ha %>%
  ...
  geom_function(
    fun = \(n) {BCI_alpha * log(1 + n / BCI_alpha)}, 
    col = "black"
  )
```
...
#| label: BCI_size
#| echo: true
# Sample `sample_size` sites in an abundance table, 
# group them and compute alpha
...
gf_ls_species <- gf_alpha_guyadiv * 
  log(1 + gf_density * gf_area / gf_alpha_guyadiv)
...
ggplot(data.frame(x = 10^(1:6)), aes(x)) +
  ...
  geom_function(
    fun = \(A) {
      gf_alpha_guyadiv * log(1 + A * gf_density / gf_alpha_guyadiv)
    }, 
    col = "blue"
  ) +
  geom_function(
    fun = \(A) {
      gf_alpha_arrhenius * log(1 + A * gf_density / gf_alpha_arrhenius)
    }, 
    col = "red"
  ) +
