...
That estimator will be denoted $\hat\alpha(\{\mathbf{\hat{p}}\})$: it corresponds to the best fit of the RAC of a log-series to the data (@fig-BCI_RAC).

When applied to the BCI dataset, both $\hat\alpha(n, S)$ and $\hat\alpha(\{\mathbf{\hat{p}}\})$ return the same estimation: `r round(BCI_alpha)`.
...
As far as we know, it is not applied in the literature: the metric of @TerSteege2023 is $\hat\alpha(n(1\ \mathbb{ha}), S)$, not $\theta$.
