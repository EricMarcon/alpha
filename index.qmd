...
keywords: [Fisher’s alpha, biodiversity, tropical forests, local scale, regional scale, log-series]
# Bibliography
...
Several R packages allow estimating $\alpha$ from a vector of abundances (see Supporting Information section 1 for details).
...
A new estimator based on goodness of fit instead of likelihood maximization is proposed in Supporting Information\@sec-appendix1: $\hat\alpha_{gof}(\{\mathbf{\hat{p}}\})$.
...
$\hat\alpha_{gof}(\{\mathbf{\hat{p}}\})$ better takes into account the rarer species (Supporting Information figure S1).
It equals 42.
...
@Hubbell2001 [page 293] proposed estimating $\theta$ by adjusting the rank-abundance curve to the most frequent species in the community, arguing that these species are also the most abundant in the metacommunity.
He found $\theta = 50$.
The same algorithm is applied to the current BCI dataset in Supporting Information section 2: its result is $\theta = 45$.
More robustly, both $\theta$ and $m$ can be estimated by likelihood maximization of the local distribution [@Etienne2005] with the untb package [@Hankin2007].
The retained value of $\theta$ is 48.
The consistency of this approach is shown in Supporting Information section 3: $\theta$ and $m$ are estimated from the BCI distribution, then a community of the same size is drawn with these parameters: it is very close to the actual distribution.
...
This theoretical result can be verified empirically with the GuyaDiv network plots in French Guiana (Supporting Information section 4) $\alpha$ increases with the number of sites included in the sample.
...
The data analysed during the current study are available elsewhere, as described in section "Example data".
The code used to produce the results and figures can be found in Supporting Information section 5.
