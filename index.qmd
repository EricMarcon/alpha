...
At regional scales, i.e. when different communities are included and beta-diversity comes into play, species-area relationships (SAR) properly speaking are derived from ecological theory with empirical support.
...
After a long period of relative obscurity, $\alpha$ made a comeback following the development of Hubbell's neutral theory since the metacommunity is distributed in log-series whose $\alpha$ equals $\theta$, the fundamental number of biodiversity.
...
-   accumulation de BCI avec log-series (nolog_nolog) : @fig-BCI_accum_logx

Fisher derived the log-series distribution from a set of assumptions.
...
#| fig-cap: Accumulatioin curve of a log-series distribution (black) and of BCI (red). The intersection of the dotted lines corresponds to the number of trees and of species of BCI 50-ha inventory.
library("tidyverse")
library("divent")
library("vegan")
data(BCI)
...
Based on @eq-fisheralpha, the species accumulation curve has two parts when it is drawn with a logarithmic X-axis: first, as $n$ is not great enough for the approximation, it is convex, then it is linear (@fig-BCI_accum_logx).

## Example data

To illustrate the estimation of Fisher's $\alpha$ at the local scale, the BCI data set available in the R package vegan will be used.
It was published by @Condit2002.
The data consist of 50 one-hectare plots of tropical evergreen forest where all tree above 10 cm DBH (diameter at breast height) were recorded.
The forest is located on Barro Colorado Island in the canal of Panama.
It has been studied by many authors, allowing to compare the results of this paper to a large literature, even though the metrics of diversity may vary somehow since the forest was inventoried at different dates, including trees above 1 cm DBH.
...
-   accumulation de BCI (nolog_nolog) : @fig-BCI_accum
-   relation alpha\~nb plots de BCI : @fig-BCI_size

```{r}
#| label: fig-BCI_accum
#| fig-cap: Species Accumulation Curve of BCI extrapolated up to twice the sample size. The dotted lines correspond to the 50-ha inventory. The horizontal, solid line, is the estimated asymptotic number of species.
BCI_accum %>%
  autoplot() +
  labs(y = "Richness") +
  theme(legend.position = "none") +
...
```{r}
#| label: fig-BCI_size
#| fig-cap: Estimation of Fisher's $\alpha$ depending on the number of 1-ha plots of BCI. The dots are random draws of plot assemblages. The line is the fit of a linear model relating $\alpha$ to the logarithm of the number of plots.
...
alphas <- replicate(
  simulations_n, 
  # Calulate alpha at all sample sizes
...
Several examples can be found in the literature, e.g. !!C
ondit, Duque, where the dependence of $\alpha$ to sample size is explored either with a statistical or an ecological approach.
...
He found $\theta = 50$.
The same algorithm is applied to the current BCI dataset in !!a
ppendix: its result is $\theta = 45$.
Both $\theta$ and $m$ can be estimated by likelihood maximization of the local distribution.
The retained value of $\theta$ is 48.
The consistency of this approach is shown in !!a
ppendix: $\theta$ and $m$ are estimated from the BCI distribution, then a community of the same size is drawn with these parameters: it is very close to the actual distribution.
...
-   Arrhenius logx : @fig-arrhenius_logx
-   relation alpha\~nb plots de Guyadiv : @fig-BCI_size

The regional scale is that of Arrhenius's model.
...
Whereas the computation of the local species accumulation curve (!!figure) was non-parametric, based solely on statistical techniques, the regional species-area relationship follows a power model with parameters c and z,
...
Assuming that forest communities are saturated, the number of individuals is proportional to the area, and the model can be rewritten in a linear form:
...
The actual size $J_M$ of the metacommunity a local community results from is difficult to address (!!reference Hubbell) and of little interest in the theory since the important parameter is $\theta$, that combines both $J_m$ and the speciation rate $\nu$.
The important fact here is that, according to the neutral model, metacommunities are distributed in log-series whose $\alpha$ parameter asymptotically tends to $\theta$ when their size increases.
...
This model is actually identical to Fisher's.

!!Support théorique et empirique pour Arrhenius.
...
```{r}
#| label: fig-arrhenius_logx
#| fig-cap: Number of tree species depending on the area (in square km, with density 627 trees/ha) in French Guiana, according to the Arrhenius's power model estimated by @Marcon2024. The Y-axis is not in logarithm, so the relation is a convex curve instead of a straight line. The black dotted lines show the valid estimation of richness in the whole territory: around 2200 species for 80,000 km^2^. The red dotted lines correspond to the number of species (around 4300) estimated by the extrapolation of the log-series sampled by the GuyaDiv network. The corresponding area, obtained by extrapolating the power model, is !!greater the whole Amazon forest. The green line is the species accumulation curve of the log-series estimated from the GuyaDiv metacommunity assuming its area is that of French Guiana. It overestimates the regional biodiversity at all scales. The red curve is the SAC of the actual metacommunity of French Guiana, whose Fisher's $\alpha$ is twice smaller.
# Read the data
# Plots
"data/plots.csv" %>% 
  read_csv2() ->
...
ggplot(data.frame(x = 10^(1:8)), aes(x)) +
  geom_function(fun = \(A) c * A^z) +
  scale_x_log10() +
  geom_vline(
    xintercept = c(gf_area, exp(log(gf_ls_species / c) / z)), 
    linetype = 2, 
    color = 1:2
  ) +
  geom_hline(
    yintercept = c(c * gf_area^z, gf_ls_species), 
    linetype = 2, 
    color = 1:2
  ) +
...
```{r}
#| label: fig-guyadiv_alpha
#| fig-cap: Estimation of the metacommunity's $\alpha$ as a function of the number of GuyaDiv plots used to sample it. The line is the fit of the linear model relating $\alpha$ to the logarithm of the number of plots.
...
!!Fin de l'argumentation: on ne peut pas utiliser alpha pour comparer des régions parce qu'on ne maîtrise pas l'étendue
...
According to the neutral theory, log-series communities would result from migration parameter 1, i.e. in the absence of dispersal limitation of the species of the metacomnunity, which never occurs.
Forcing the estimation of $\alpha$ in non-log-series distributions results in highly scale-dependent values that seriously questions the comparisons between communities based on their $\alpha$ index.
...
-   3 représentations selon les logs, valeur de la pente
