...
A new estimator based on goodness of fit instead of likelihood maximization is proposed in Appendix 1: $\hat\alpha_{gof}(\{\mathbf{\hat{p}}\})$.
...
$\hat\alpha_{gof}(\{\mathbf{\hat{p}}\})$ better takes into account the rarer species (Figure S1).
It equals 42.
...
alphas <- replicate(
  simulations_n, 
  # Calculate alpha at all sample sizes
...
@Hubbell2001 [page 293] proposed estimating $\theta$ by adjusting the rank-abundance curve to the most frequent species in the community, arguing that these species are also the most abundant in the metacommunity. He found $\theta = 50$. The same algorithm is applied to the current BCI dataset in Appendix 2: its result is $\theta = 45$. More robustly, both $\theta$ and $m$ can be estimated by likelihood maximization of the local distribution [@Etienne2005] with the untb package [@Hankin2007].
...
This theoretical result can be verified empirically with the GuyaDiv network plots in French Guiana (Appendix 4): $\alpha$ increases with the number of sites included in the sample.
