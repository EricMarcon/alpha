...
After a long period of relative obscurity, $\alpha$ made a comeback folowing the development of Hubbell's neutral theory since the metacommunity is distributed in log-series whose $\alpha$ equals $\theta$, the fundamental number of biodiversity.
...
Then, the number of species with non-zero abundance $S(n)$ is linked to the size of the community $n$ through equation @eq-fisheralpha by a parameter named $\alpha$.
...
Based on @eq-fisheralpha, the species accumulation curve has two parts when it is drawn with a logarithmic X-axis: first, as $n$ is not great enough for the approximation, it is convex, then it is linear (figure @fig-BCI_accum_logx).
...
Figure @fig-BCI_accum shows the SAC of the Barro Colorado Island 50-ha forest plot based on the data provided by the *vegan* R package.
...
@Gleason1922 refuted Arrhenius' model, arguing that the richness would be far too high for large areas, and proposed a competing model in which the number of species, rather than its logarithm, increased with the logarithm of the area.
...
When the SAR is plotted on a log-log scale (!!figure), it is linear.
If the Y-axis is not in logarithmic scale, it becomes concave.
In this representation (!!fig), the parameter $\alpha$ of log-series increases with area (!! pas limpide exprimé de cette façon).
...
In figure @fig-guyadiv_alpha, an increasing number of plots (one per site) is randomly selected to sample the metacommunity.
...
alphas <- sapply(sizes, FUN = guyadiv_sub_alpha)
...
According to the neutral theory, log-series communities would result from migration parameter 1, i.e. in the absence of dispersal limitation of the species of the metacommnunity, which never occurs.
